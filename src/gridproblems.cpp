...
namespace gridprob
{
    ...
    std::vector<ZRootTwo> gridpoints_internal(T x0, T x1, T y0, T y1)
    {
        ...
        if (dy == 0 && dx > 0)
        {
            std::vector<ZRootTwo> sub_results = gridpoints_internal(y0, y1, x0, x1);
            std::vector<ZRootTwo> results(sub_results.size());
        ...
        {
            std::vector<ZRootTwo> sub_results = gridpoints_internal(
    ...
    std::vector<ZRootTwo> gridpoints(T x0, T x1, T y0, T y1)
    {
        ...
        std::vector<ZRootTwo> raw_points = gridpoints_internal(x0new, x1new, y0new, y1new);
        std::vector<ZRootTwo> shifted(raw_points.size());
    ...
    template <typename T>
    std::vector<DRootTwo> gridpoints_scaled(std::tuple<T, T> intX, std::tuple<T, T> intY, Integer k)
    {
        T x0 = std::get<0>(intX);
        T x1 = std::get<1>(intX);
        T y0 = std::get<0>(intY);
    ...
    std::vector<DRootTwo> gridpoints_scaled_parity(DRootTwo beta, T x0, T x1, T y0, T y1, Integer k)
    {
        ...
        std::vector<DRootTwo> z = gridpoints_scaled(x0 + offs_prime, x1 + offs_prime, y0 + offs_bul_prime, y1 + offs_bul_prime, k - 1);
    ...
    template <typename T>
    std::tuple<T, T> operator_to_bl2z(Operator<T> op)
    {
        T a = op(0, 0);
        T b = op(0, 1);
        T d = op(1, 1);
        ...
        ConvexSet<T> setA, ConvexSet<T> setB, Operator<DRootTwo> opG)
    {
        ...
        std::function<std::vector<DOmega>(Integer)> solutions_fun = [=](Integer k)
        {
            ...
            std::vector<DRootTwo> xs = gridpoints_scaled<T>(
            ...
            std::vector<DRootTwo> beta_prime_list = gridpoints_scaled(
            ...
            for (DRootTwo beta_prime : beta_prime_list)
            {
                ...
                std::vector<DRootTwo> alpha_prime_offs_list = gridpoints_scaled_parity(
